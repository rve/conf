...
struct network
{
    ...
    void addEdge(int from, int to, int capacity, long cost)
    {
        ...
        typeof( adj[from].find(k) ) q = adj[from].find(k);
        if(q!=adj[from].end()) {
            q->second+=capacity;
        } else {
            adj[from][k]=capacity;
...
int main(){
    ...
    REP_1(i , n){
        ...
        for (int j = 2 ; j <= a[i][0] ; ++j){
            if (dsu::find(a[i][j]) != dsu::find(a[i][1]))
                dsu::unionset(dsu::find(a[i][j]) , dsu::find(a[i][1]));
        }
    }
    for (int i = 1 ; i <= m ; ++i)if(ex[i]){
        if (dsu::find(i) != dsu::find(x)){
            ++ans;
            dsu::unionset(dsu::find(i) , dsu::find(x));
        }
    }
    OT(ans);
}
